...
> **Atualizado em 01/06/2026:** Todos os controllers agora usam DTOs próprios de request e response. Nenhuma entidade JPA é exposta diretamente na API.
...
> Modelo de **duas pessoas** (princípio dos quatro olhos): o tesoureiro usa o app e o conferente assina presencialmente.

| Campo | Label | Tipo | Obrigatório | Validação |
|---|---|---|---|---|
| `tesoureiroId` | — (enviado automaticamente) | UUID | ✅ | ID do usuário logado; preenchido pelo app |
| `conferenteNome` | Nome do Conferente | text | ✅ | 2–100 caracteres; segunda pessoa presente |
| `conferenteId` | — (selecionar conferente) | select/UUID | não | ID de outro usuário do sistema (se tiver conta) |
| `nomesTesoureiros` | Outros participantes | text | não | Texto livre para registros adicionais |
...
| `conferenteNome` | Vazio | "O nome do conferente é obrigatório" |
| `conferenteNome` | Menos de 2 chars | "O nome do conferente deve ter entre 2 e 100 caracteres" |
...
**Request:** `MembroRequestDTO` | **Response:** `MembroResponseDTO`

#### Campos de Input (`MembroRequestDTO`)
...
#### Campos da Response (`MembroResponseDTO`)
...
| `dataMembresia` | date | Data de ingresso |
| `endereco` | string | Endereço |
| `status` | enum | Status atual |
| `dizimista` | boolean | Flag dizimista |
| `observacoes` | string | Observações |

> `igrejaId`, `version` e campos internos de persistência **não são retornados**.
